...
//Images
import AgentImage from './images/agent_centered_sm.png';
...
//Right-Click menu
// From the agent selected, with a right-click event you display a menu to do the following:
...
function menu(e1) {
	...
	if (e1.nodes.length === 1) {
		...
		contextMenu.innerHTML = '<div class="item" id="nameAgent">' + name + '</div><div class="topicmenu" id="topic"></div><div class="item" id = "webpage"></div>'
		...
		document.getElementById('webpage').innerHTML = '<button onclick="window.open(&#34;https://openebench.bsc.es/agent/' + label + '&#34; , &#34;_blank&#34; )">Webpage</button>';
				...
				element.graph.nodes.forEach(nodeElement =>{
					if (!idNodesSet.has(nodeElement.id)){
						...
						if(nodeElement.labels[0]==='Publication'){
							...
							var imageLabel;
							if(nodeElement.labels[0]==='Agent'){ 
								imageLabel = AgentImage
							}
...
const agentName = urlParams.get('agent') || 'No word provided';
const limitAPI = urlParams.get('limit') || 10;

addLogo();
drawVis();
var cypherQuery = 'MATCH (i)-[o:METAOCCUR_ALL]-(p) where i.name="' + agentName +'" return i,o,p order by o.times limit ' + limitAPI;
...
// function addLoadingAgent (){ 

// 	Vis.stopSimulation();
// 	Vis.off('afterDrawing', addLoadingAgent);
// 	// Vis.network.fit();
// 	Vis.stopSimulation();
...
// function waitAddAgent(){
// 	setTimeout(function(){
// 		Vis.stabilize(100);
// 		Vis.on('afterDrawing', addLoadingAgent);
// 	})
// }
